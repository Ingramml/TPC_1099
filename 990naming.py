...
files = glob.glob('/Volumes/SSD/TPC990/Amazon_XMLs/*/*.xml')
...
for i in tqdm(files):
    ...
    begindate = begindate_check.text if ET.iselement(begindate_check) else begindate_chcek2.text if ET.iselement(begindate_chcek2)==True else ''
    year = begindate[0:4]
    ...
    if os.path.isfile(filecheck):
        ...
        if ET.iselement(ScheduleI) == True and ET.iselement(ScheduleI.find('{http://www.irs.gov/efile}RecipientTable')):
            ...
            for recipient in RecipientTablelist:
                ...
                business_name_check3 = recipient.find('./*/{http://www.irs.gov/efile}BusinessNameLine1') #2011
            ...
            for recipient in grants_list:
                ...
                business_name_check = recipient.find('./*/{http://www.irs.gov/efile}BusinessNameLine1Txt')
                business_name_check2 = recipient.find('./{http://www.irs.gov/efile}RecipientPersonNm')
                business_name_check3 = recipient.find('./*/{http://www.irs.gov/efile}BusinessNameLine1') #20113
                business_name = business_name_check.text if ET.iselement(
                    business_name_check) == True else business_name_check3.text if ET.iselement(
                    business_name_check3) == True else business_name_check2.text if ET.iselement(
                    business_name_check2) == True else ''
                ...
                #business_name_check2_ln2 = recipient.find('./{http://www.irs.gov/efile}RecipientPersonNm')
                business_name_check3_ln2 = recipient.find('./*/{http://www.irs.gov/efile}BusinessNameLine2')  # 20113
                business_name_ln2 = business_name_check_ln2.text.upper() if ET.iselement(
                    business_name_check_ln2) == True else business_name_check3.text if ET.iselement(
                    business_name_check3_ln2) == True else  ''

                org_name = business_name +" "+ business_name_ln2
               ...
               # status_element_check = recipient.find('./{http://www.irs.gov/efile}Status')
                ...
                rows.append([GrantorEIN, recipient_EIN, org_name, amount_element ,purpose_element])

            df = pd.DataFrame(rows, columns=["grantor_EIN", "grantee_ein", "organization", "amount", "purpose"])
            df.to_csv(filecheck)
        else:
            rows = []
            df = pd.DataFrame(rows, columns=["grantor_ein", "grantee_ein", "organization", "amount", "purpose"])  # creates df of orgs donations
            df.to_csv(target_location + '/'+ year+ '/grants' + GrantorEIN + '.csv')
        df2 = pd.DataFrame(rows2, columns=['EIN', 'Docnumber'])